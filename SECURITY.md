...
- **Third-party CORS proxies.** Import features (Cheesefork ICS, YouTube playlists, Panopto folders) route requests through a CORS proxy because those hosts don't send permissive CORS headers. In development this is a same-origin dev-server proxy **restricted to the specific import hosts**; the production static build falls back to public proxies (allorigins / codetabs). A link you paste to import therefore transits a third-party proxy — avoid pasting links that embed private tokens. Catalog enrichment fetches the public Technion dataset from GitHub directly.
